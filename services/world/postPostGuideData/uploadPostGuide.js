...
async function loadAndPatchUpdatePostGuideData() {
  try {
    // 최신 파일을 얻기 위해 패턴을 사용하여 파일 찾기
    const dataDir = path.join(__dirname, '../rawData');
    const files = await fs.readdir(dataDir);
    ...
    console.log(`Loaded ${Array.isArray(postGuidesData) ? postGuidesData.length : 1} post guide(s) from ${postGuideDataFile} for patch update`);
    
    // 부분 업데이트 함수 사용 (필요한 필드만 지정 가능)
    const fieldsToUpdate = process.argv.includes('--fields') ? 
      process.argv[process.argv.indexOf('--fields') + 1]?.split(',') : null;
    
    const result = await PostGuideUploadService.patchUpdateMultiplePostGuides(postGuidesData, fieldsToUpdate);
    console.log('Patch update completed:', result);
    process.exit(0);
  } catch (error) {
    console.error('Error in load and patch update process:', error);
    process.exit(1);
  }
}
...
if (require.main === module) {
  ...
  if (mode === 'update') {
    ...
    loadAndPatchUpdatePostGuideData();
  } else {
    loadAndUploadPostGuideData();
  }
}

module.exports = { loadAndUploadPostGuideData, loadAndUpdatePostGuideData, loadAndPatchUpdatePostGuideData };